...
export default function BathroomEstimator({
	...
	useEffect(() => {
		// Defers the state update out of the synchronous render cycle to fix the cascading render warning
					...
					<div className="grid sm:grid-cols-2 gap-6">
						...
						<div className="space-y-2">
							...
							<select
								value={vanity}
								onChange={e =>
									setVanity(e.target.value as BathroomSpecs['vanityConfig'])
								}
						...
						<div className="space-y-2">
							...
							<select
								value={wetArea}
								onChange={e =>
									setWetArea(e.target.value as BathroomSpecs['wetAreaType'])
								}
								className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500"
							>
...
// Added interface here instead of using 'any'
...
function PriceTier({ title, price, desc, featured = false }: PriceTierProps) {
	return (
		<div
